...
// "privacy") at request time and falls back to the hardcoded copy below
// only if no row exists yet or its contentHtml is empty - see
// get-static-page.ts and static-pages.service.ts's own DEFAULT_STATIC_PAGE_CONTENT
// doc comment. This used to be `force-static`; it now has to hit the
// backend on every request for the admin panel's edits to actually take
// effect, so this is a deliberate build-time -> per-request tradeoff.
export const dynamic = 'force-dynamic';

const FALLBACK_LAST_UPDATED = 'August 16, 2026';
...
export default async function PrivacyPage() {
  ...
  if (override?.contentHtml) {
    ...
    return (
      <LegalPageLayout title={override.title || 'Privacy Policy'} lastUpdated={lastUpdated}>
  ...
  return (
    <LegalPageLayout title="Privacy Policy" lastUpdated={FALLBACK_LAST_UPDATED}>
      {FALLBACK_CONTENT}
    </LegalPageLayout>
  );
}
