// BuilderState <-> HTML string, ported from vantly-ugc.com's Content
// Builder (which itself ported the shape from AutoGPT's Mailer template
// builder). Every block renders as `<table>` layout (table/tbody/tr/td)
...
// or badly mis-render `display:flex`/`display:grid`. This produces the
// exact string saved as `EmailTemplate.htmlContent`, later rendered by
// mail clients when a campaign sends it -- this designer is only a
// friendlier way to produce that same string.
...
const MARKER_PREFIX = '<!--MAILER_BUILDER_STATE:';
const MARKER_SUFFIX = '-->';
const MAX_UNWRAP_DEPTH = 25;

/** Encodes a string as base64, safe for embedding inside an HTML comment.
 * Deliberately NOT raw JSON text -- raw JSON can legitimately contain a
 * literal `-->` (e.g. inside a Raw HTML block's own content), which
 * would truncate the marker early. Base64's alphabet has no `-` or `>`
 * characters, so this can never happen. */
...
// role="presentation" + cellpadding/cellspacing/border="0" on every
// layout table is the standard email-HTML convention for "this table is
// layout, not tabular data", recognized by every major email client and
// screen reader.
 ...
 * `EmailTemplate.htmlContent`. Never throws: if `JSON.stringify` somehow
 * fails, falls back to saving the body with no marker rather than
 * crashing the editor. */
 ...
 * tracking string-literal state so a `-->` inside a string value can
 * never be mistaken for structural JSON. Returns -1 if unbalanced. */
 ...
 * current format's payload is base64, which can never contain a literal
 * `-->`, so the FIRST `-->` after the prefix is always correct. Legacy/
 * corrupted-input path: falls back to a brace/bracket-and-string-aware
 * scan via `findJsonValueEnd`. */
function tryParseMarkerLayer(html: string): BuilderState | null {
  ...
  if (firstEnd !== -1) {
    try {
      ...
      // not (valid) base64 at this boundary -- fall through
    }
  }
 ...
 * double-wrapped marker so a template self-heals rather than doubling in
...
/** HTML string -> state. Only content this designer itself produced
 * (identified by the leading JSON marker comment) round-trips as real,
 * editable blocks -- anything else (a template's raw-HTML textarea
 * value, or hand-edited Source Code that changed structure) gets wrapped
 * as a single "Raw HTML" block, so nothing already saved is ever
 * silently lost. */
