// Pure state-mutation functions for the Mailer Template designer, ported
// from vantly-ugc.com's Content Builder. All mutations take a
// BuilderState, return a new one -- since the state (rows/columns/blocks)
// is plain JSON with no functions/dates/etc., a deep clone via JSON
// round-trip is simple and safe, and this builder's data is small enough
// that it's not a performance concern.
...
export function addRow(
  ...
  columnCount: 1 | 2 | 3,
  atIndex?: number
): { state: BuilderState; rowId: string } {
 ...
 * removed onto the end of the last remaining column. */
...
export function setColumnWidths(
  ...
  row.columns.forEach((col, i) => {
    ...
    col.widthPercent = Math.min(
      90,
      Math.max(MIN_COLUMN_WIDTH_PERCENT, widths[i])
    );
  });
  return next;
}
 ...
 * of columns flanking the dragged handle change width. */
...
/** Rewrites every block's *accent* color(s) to the given theme -- row
 * background colors and Text block colors are deliberately untouched
 * (often customized per-row; silently overwriting them on every theme
 * click would be more surprising than helpful). */
 ...
 * within the same column" and "drag into a different column". */
