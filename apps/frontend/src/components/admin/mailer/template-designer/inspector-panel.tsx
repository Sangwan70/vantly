...
// Right-hand settings panel for the Mailer Template designer's selected
// row/block, ported from vantly-ugc.com's Content Builder InspectorPanel.tsx.

import { Block, BlockAlign, BuilderState, Row, Selection, SocialPlatform, newId } from '@gitroom/frontend/lib/mailer-builder/types';
import { BuilderActions } from '@gitroom/frontend/lib/mailer-builder/use-builder-actions';
import { MiniButton, MiniInput, MiniSelect, MiniTextarea } from './mini-ui';
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-[6px]">
      <label className="text-[11px] opacity-60">{label}</label>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-[8px]">
      <input
        ...
        className="h-[28px] w-[36px] rounded border border-newTableBorder bg-transparent"
      />
      <MiniInput value={value} placeholder={allowClear ? 'inherit' : '#000000'} onChange={(e) => onChange(e.target.value)} />
      {allowClear && value && <MiniButton onClick={() => onChange('')}>Clear</MiniButton>}
    </div>
  );
}
...
export function InspectorPanel({ state, selection, actions }: { state: BuilderState; selection: Selection; actions: BuilderActions }) {
  if (!selection) {
    return <div className="p-[16px] text-[12px] opacity-60">Select a row or block to see its settings here.</div>;
  }
  ...
  if (selection.kind === 'row') {
    ...
    if (!row) return null;
    return (
      <div className="space-y-[16px] p-[12px]">
        <h3 className="text-[11px] font-[600] uppercase opacity-60">Row settings</h3>
        <Field label="Columns">
          <div className="flex gap-[4px]">
          ...
          <p className="text-[10px] opacity-60">Reducing columns moves their blocks into the last remaining column -- nothing is deleted.</p>
        </Field>
            ...
            <div className="space-y-[6px]">
              {row.columns.map((col, i) => (
                <div key={col.id} className="flex items-center gap-[8px]">
                  <span className="w-[16px] shrink-0 text-[10px] opacity-60">{i + 1}</span>
                  <MiniInput
                    type="number"
                    min={10}
                    max={90}
              ...
              <p className="text-[10px] opacity-60">
                Total: {Math.round(row.columns.reduce((sum, c) => sum + c.widthPercent, 0) * 10) / 10}% -- doesn&apos;t need to be
                exactly 100. Or drag the divider between columns directly on the canvas.
              </p>
  ...
  return (
    <div className="space-y-[16px] p-[12px]">
      <h3 className="text-[11px] font-[600] uppercase opacity-60">{block.type} block settings</h3>
      ...
      {block.type === 'image' && (
        <>
          <Field label="Width (px)">
            ...
            <p className="text-[10px] opacity-60">0 = fills the column. Or drag the handle on the image.</p>
          </Field>
          <Field label="Alignment">
            <AlignField value={block.align} onChange={(align) => actions.updateBlock(block.id, { align })} />
          </Field>
              ...
              {block.links.map((link, i) => (
                <div key={link.id} className="flex items-center gap-[6px]">
                  ...
                  {block.links.length > 1 && (
                    <button
                      type="button"
                      className="shrink-0 rounded border border-newTableBorder p-[6px] text-red-400 hover:bg-tableBorder"
                      title="Remove link"
                      onClick={() => {
      ...
      {block.type === 'raw' && <p className="text-[12px] opacity-60">Edit this block&apos;s markup directly in the canvas -- it&apos;s selected there now.</p>}
